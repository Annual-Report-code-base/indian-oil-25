...
const NavMenu = () => {
  ...
  const handleMenuClick = (section, item) => {
    if (section.isInternal) {
      // Internal navigation using react-router Link (simulated)
      console.log(`Navigate to: ${item.name} - Page ${item.page}`);
    } else {
      // External PDF link
      const pdfUrl = `#page=${item.page}`;
        ...
        <div className="bg-gray-100 py-2">
          {subSection.items.map((item, index) => (
            <button
              ...
              className="w-full px-10 py-2.5 text-left text-gray-600 hover:bg-white hover:text-blue-600 transition-all duration-200 flex items-center justify-between group"
            >
              <span className="text-sm">{item.name}</span>
              <div className="flex items-center space-x-2">
                <span className="text-xs text-gray-500">Page {item.page}</span>
                <HiChevronRight size={12} className="text-gray-400 group-hover:text-blue-600 transition-colors" />
              </div>
            </button>
          ))}
        </div>
    ...
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      >
        <div className="bg-gray-50 py-2">
          {/* Handle sections with sub-accordions */}
          {section.hasSubSections ? (
            <>
              {/* Render sub-accordions */}
              ...
              {/* Render additional items that don't belong to sub-sections */}
            ...
            /* Handle regular sections without sub-accordions */
      ...
      <header className="bg-white shadow-md fixed top-0 left-0 right-0 z-40 w-full">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center w-35">
           <img src="./Indian Oil Microsite Image 2025/Indian Oil Logo@4x.webp" alt="" />
          </div>
